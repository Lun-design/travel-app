...
export default function TripDetailScreen() {
  ...
  return <View style={[styles.container, { paddingHorizontal: layout.screenPaddingHorizontal, paddingTop: layout.screenPaddingTop, paddingBottom: 22 + insets.bottom }]}>
    <View style={[styles.header, { paddingTop: insets.top }]}>
      <View style={styles.headerTitleRow}>
        <View style={styles.titleCopy}>
          <Pressable onPress={() => router.canGoBack() ? router.back() : router.replace('/')}><Text style={styles.back}>‹ 返回我的行程</Text></Pressable>
          <Text style={styles.title}>{trip.title}</Text>
        </View>
        <PuppyMascot puppy="-11" size={headerMascotSize} style={styles.headerMascot} accessibilityLabel="旅伴一起出發" />
      </View>
      <View style={styles.headerMetaRow}>
        <View style={styles.dateBlock}><Text style={styles.destination}>{trip.destination} · {trip.start_date} ～ {trip.end_date}</Text></View>
        <View style={styles.headerMembers}>{members.slice(0, 4).map((member, index) => <View key={member.user_id} style={[styles.avatar, { marginLeft: index ? -9 : 0 }]}><Text style={styles.avatarText}>{(member.profile?.display_name || member.user_id)[0].toUpperCase()}</Text></View>)}<Pressable style={styles.inviteButton} onPress={() => setInviteVisible(true)}><Text style={styles.inviteText}>＋ 邀請</Text></Pressable></View>
    ...
    <View style={styles.tabShell}>
      ...
      <View pointerEvents="none" style={styles.tabScrollHint}><Text style={styles.tabScrollHintText}>›</Text></View>
    </View>
...
const styles = StyleSheet.create({
  ...
  headerTitleRow: { width: '100%', maxWidth: '100%', flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', gap: 12, minWidth: 0 },
  titleCopy: { flex: 1, minWidth: 0 },
  headerMascot: { flexShrink: 0 },
  headerMetaRow: { width: '100%', maxWidth: '100%', flexDirection: 'row', alignItems: 'center', flexWrap: 'wrap', gap: 8, minWidth: 0 },
  ...
  settingsButton: { flexShrink: 0, borderRadius: 12, backgroundColor: '#e0e7ff', paddingHorizontal: 12, paddingVertical: 8 },
  settingsText: { color: '#3730a3', fontWeight: '700' },
  offlineBar: { width: '100%', maxWidth: '100%', minHeight: 32, justifyContent: 'center', borderRadius: 10, backgroundColor: '#fef3c7', paddingHorizontal: 12, paddingVertical: 6, marginBottom: 8, overflow: 'hidden' },
  offlineText: { color: '#92400e', fontSize: 12, fontWeight: '700' },
  tabShell: { width: '100%', maxWidth: '100%', minHeight: 44, height: 44, flexShrink: 0, position: 'relative', overflow: 'hidden', marginBottom: 7 },
  tabScroller: { width: '100%', maxWidth: '100%', minHeight: 44, height: 44, flexGrow: 0, flexShrink: 0 },
  ...
  tabScrollHintText: { color: '#64748b', fontSize: 23, fontWeight: '900', lineHeight: 24 },
  ...
  mapToggle: { width: '100%', maxWidth: '100%', minHeight: 44, alignItems: 'center', justifyContent: 'center', borderRadius: 14, backgroundColor: '#e0e7ff', paddingHorizontal: 14, paddingVertical: 10, marginBottom: 10, overflow: 'hidden' },
