...
export default function TripDetailScreen() {
  ...
  if (data.loading && !data.trip) return <View style={[styles.loadingShell, { backgroundColor: theme.colors.background }]}><SkeletonCard variant="header" /><SkeletonCard /><SkeletonCard /></View>;
  ...
  return <ActiveTripContext.Provider value={trip.id}><View style={[styles.container, { backgroundColor: theme.colors.background, paddingHorizontal: layout.screenPaddingHorizontal, paddingTop: layout.screenPaddingTop, paddingBottom: 22 + insets.bottom }]}>
    ...
    {tab === 'timeline' ? <View style={styles.importBar}><Pressable accessibilityRole="button" style={[styles.importButton, { borderColor: theme.colors.border, backgroundColor: theme.colors.surface }]} onPress={() => { setImportAction('import'); setImportVisible(true); }}><Text style={{ color: theme.colors.primary, fontWeight: '800' }}>📥 一鍵匯入行程</Text></Pressable><Pressable accessibilityRole="button" style={styles.importButton} onPress={() => { setImportAction('clear'); setImportVisible(true); }}><Text style={{ color: theme.colors.muted }}>清空全行程景點</Text></Pressable></View> : null}
    {tab === 'timeline' && <TimelinePanel key={refreshKey} trip={trip} day={day} days={days} items={data.items} visibleItems={visibleItems} themeMode={themeMode} layout={layout} insets={insets} isMapOpen={isMapOpen} isMapLoading={isMapLoading} isDayTransitioning={isDayTransitioning} focusedItemId={focusedItemId} vouchers={data.vouchers} timelineScrollRef={timelineScrollRef} onDayChange={handleDayChange} onToggleMap={toggleMap} onMapMarkerPress={handleMapMarkerPress} onFocusedVoucher={setPreviewVoucher} onSwitchToBackupPlan={handleSwitchToBackupPlan} onEdit={(item) => { setEditingItem(item); setInsertPosition(null); setItemModal(true); }} onDelete={deleteItem} onReorder={data.reorderItems} onApplyRouteOptimization={applyRouteOptimization} onAddAtPosition={(position) => { setEditingItem(null); setInsertPosition(position); setItemModal(true); }} onAdd={() => { setEditingItem(null); setInsertPosition(null); setItemModal(true); }} />}
