...
import { mapDraftToTargetTrip, type ImportedItineraryPayload, type ImportedTripDraft } from '@/lib/itinerary-import';
...
export default function TripDetailScreen() {
  ...
  const [importAction, setImportAction] = useState<'import' | 'clear'>('import');
  ...
  async function importIntoTrip(draft: ImportedTripDraft, targetTripId: string, dayOffset: number, previewPayloads: ImportedItineraryPayload[], mode: ImportMode = 'merge', destination = ''): Promise<ImportResult> {
    ...
    return { targetTripId, ...result, failed: 0 };
  }
  async function clearAllItems(): Promise<ImportResult> {
    const result = await clearTripItems(trip.id);
    data.setItems([]);
    await data.reload();
    setRefreshKey(current => current + 1);
    setDay(1);
    return { targetTripId: trip.id, ...result, failed: 0 };
  }
  ...
  if (data.loading && !data.trip) return <View style={[styles.loadingShell, { backgroundColor: theme.isDark ? theme.colors.background : '#F7F5F0' }]}><SkeletonCard variant="header" /><SkeletonCard /><SkeletonCard /></View>;
  ...
  return <ActiveTripContext.Provider value={trip.id}><View style={[styles.container, { backgroundColor: theme.isDark ? theme.colors.background : '#F7F5F0', paddingHorizontal: layout.screenPaddingHorizontal, paddingTop: layout.screenPaddingTop, paddingBottom: 22 + insets.bottom }]}>
    ...
    <ItineraryImportModal visible={importVisible} initialAction={importAction} onClear={clearAllItems} currentTripId={trip.id} trips={importTrips.length ? importTrips : [{ id: trip.id, title: trip.title, start_date: trip.start_date, end_date: trip.end_date }]} existingItems={data.items} themeMode={themeMode} onClose={() => setImportVisible(false)} onConfirm={importIntoTrip} />
...
const styles = StyleSheet.create({
  ...
  realtimeBar: { width: '100%', minHeight: 32, justifyContent: 'center', borderRadius: 10, borderWidth: 1, paddingHorizontal: 12, paddingVertical: 6, marginBottom: 8, overflow: 'hidden' },
