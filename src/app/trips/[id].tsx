...
export default function TripDetailScreen() {
  ...
  return <ActiveTripContext.Provider value={trip.id}><View style={[styles.container, { backgroundColor: theme.colors.background, paddingHorizontal: layout.screenPaddingHorizontal, paddingTop: layout.screenPaddingTop, paddingBottom: 22 + insets.bottom }]}>
    ...
    {tab === 'places' && <TripPlacesPanel tripId={tripId!} userId={data.userId} places={data.places} days={days} themeMode={themeMode} onChanged={refreshPlaces} onScheduled={handlePlaceScheduled} onTimezoneDetected={async (timezone) => { if (timezone !== trip.timezone) { await data.saveTripSettings({ timezone }); await data.reload(); } }} />}
    </MainScroll>
