...
export default function PublicShareScreen() {
  ...
  async function handleClone() {
    if (!payload || !token || cloneBusy) return;
    setCloneNotice('');
    const session = await supabase.auth.getSession().then(({ data }) => data.session).catch(() => null);
    if (!session) {
      setCloneNotice('請先登入，再將這份行程複製到你的帳號。');
      router.push({ pathname: '/login', params: { redirect: `/share/${encodeURIComponent(token)}` } });
      return;
    }
    setCloneBusy(true);
    try {
      const clonedTripId = await cloneTripById(payload.trip.id, { shareToken: token });
      router.replace(`/trips/${clonedTripId}`);
    } catch (cause) {
      setCloneNotice(cause instanceof Error ? cause.message : '目前無法複製這份行程。');
    } finally {
      setCloneBusy(false);
    }
  }
  ...
  return <ScrollView style={{ backgroundColor: theme.colors.background }} contentContainerStyle={styles.page}>
    <View style={styles.hero}><Text style={styles.eyebrow}>SHARED ITINERARY</Text><Text style={[styles.title, { color: theme.colors.text }]}>{payload.trip.title}</Text><Text style={[styles.destination, { color: theme.colors.muted }]}>{payload.trip.destination} · {payload.trip.start_date} – {payload.trip.end_date}</Text><Text style={[styles.readonly, { color: theme.colors.primary, backgroundColor: theme.colors.surfaceMuted }]}>唯讀分享</Text><Pressable style={[styles.cloneButton, { backgroundColor: theme.colors.primary }]} onPress={() => void handleClone()} disabled={cloneBusy}><Text style={styles.cloneButtonText}>{cloneBusy ? '正在複製…' : '📋 複製此行程到我的帳號'}</Text></Pressable>{cloneNotice ? <Text style={[styles.cloneNotice, { color: theme.colors.muted }]}>{cloneNotice}</Text> : null}</View>
...
const styles = StyleSheet.create({
  ...
  cloneButton: { alignSelf: 'flex-start', minHeight: 44, justifyContent: 'center', borderRadius: 10, paddingHorizontal: 14, paddingVertical: 10 },
  cloneButtonText: { color: '#FFFDF8', fontWeight: '800' },
  cloneNotice: { fontSize: 13, lineHeight: 19 },
});
