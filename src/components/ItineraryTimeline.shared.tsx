...
export function TimelineCard({ item, segment, scheduled, weather, vouchers, onPreviewVoucher, grip, active, onEdit, onDelete, onMoveUp, onMoveDown, canMoveUp, canMoveDown }: TimelineCardProps) {
          ...
          <View style={styles.cardBody}>
            ...
            <View style={styles.content}>
              <View style={styles.cardHeader}>
                <Text style={[styles.time, { color: theme.colors.primary }]}>{scheduled?.arrivalTime ?? item.time ?? '未排定'}{scheduled?.estimated ? ' · 預估' : ''}</Text>
              ...
              <View style={styles.actions}>
                {onMoveUp ? <Pressable accessibilityRole="button" accessibilityLabel="上移景點" disabled={!canMoveUp} onPress={onMoveUp}><Text style={[styles.reorderText, !canMoveUp && styles.disabledAction]}>▲ 上移</Text></Pressable> : null}
                {onMoveDown ? <Pressable accessibilityRole="button" accessibilityLabel="下移景點" disabled={!canMoveDown} onPress={onMoveDown}><Text style={[styles.reorderText, !canMoveDown && styles.disabledAction]}>▼ 下移</Text></Pressable> : null}
                <Pressable onPress={() => onEdit(item)}><Text style={styles.edit}>編輯</Text></Pressable>
                <Pressable onPress={() => onDelete(item)}><Text style={styles.delete}>刪除</Text></Pressable>
                {itemVouchers.length > 0 && onPreviewVoucher ? <Pressable onPress={() => onPreviewVoucher(itemVouchers[0])}><Text style={styles.voucher}>🎫 檢視票券{itemVouchers.length > 1 ? ` (${itemVouchers.length})` : ''}</Text></Pressable> : null}
...
const styles = StyleSheet.create({
  empty: { width: '100%', padding: 38, alignItems: 'center', gap: 8, boxSizing: 'border-box' }, emptyText: { color: '#64748b' }, row: { width: '100%', flexDirection: 'row', minHeight: 130 }, rail: { width: 24, alignItems: 'center' }, line: { position: 'absolute', top: 18, bottom: 0, width: 2, backgroundColor: '#dbeafe' }, dot: { width: 12, height: 12, borderRadius: 6, backgroundColor: '#2563eb', borderWidth: 3, borderColor: '#dbeafe', zIndex: 1 }, card: { flex: 1, minWidth: 0, maxWidth: '100%', overflow: 'hidden', boxSizing: 'border-box', marginBottom: 12, padding: 12, borderRadius: 15, borderWidth: 1, position: 'relative' }, cardActive: { borderColor: '#60a5fa', shadowColor: '#0f172a', shadowOpacity: 0.16, shadowRadius: 12, elevation: 5 }, cardBody: { width: '100%', flexDirection: 'row', alignItems: 'stretch', gap: 10 }, grip: { width: 32, minHeight: 76, alignItems: 'center', justifyContent: 'center', borderRadius: 10, backgroundColor: '#e2e8f0' }, gripText: { color: '#64748b', fontSize: 25, fontWeight: '900' }, content: { flex: 1, minWidth: 0, gap: 5 }, cardHeader: { width: '100%', flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', gap: 6 }, time: { fontWeight: '800' }, category: { fontSize: 12, flexShrink: 1 }, categoryWrap: { flexDirection: 'row', alignItems: 'center', gap: 4, flexShrink: 1 }, inlineMascot: { flexShrink: 0 }, weatherRow: { flexDirection: 'row', alignItems: 'center', flexWrap: 'wrap', gap: 8 }, weatherText: { fontSize: 12, fontWeight: '700' }, rainProbability: { color: '#2563eb', fontSize: 12, fontWeight: '800' }, weatherAlerts: { gap: 4 }, weatherWarning: { color: '#075985', backgroundColor: '#e0f2fe', borderRadius: 7, paddingHorizontal: 8, paddingVertical: 4, fontSize: 12, fontWeight: '800' }, extremeWarning: { color: '#9a3412', backgroundColor: '#ffedd5', borderRadius: 7, paddingHorizontal: 8, paddingVertical: 4, fontSize: 12, fontWeight: '800' }, name: { fontSize: 18, fontWeight: '800' }, duration: { fontSize: 13 }, warningStack: { position: 'absolute', top: 10, right: 10, zIndex: 2, alignItems: 'flex-end', gap: 4, maxWidth: '72%' }, openingWarning: { borderRadius: 7, paddingHorizontal: 8, paddingVertical: 4, fontSize: 12, fontWeight: '800' }, overlapWarning: { color: '#991b1b', backgroundColor: '#fee2e2', borderRadius: 7, paddingHorizontal: 8, paddingVertical: 4, fontSize: 12, fontWeight: '800' }, address: { fontSize: 13 }, notes: { fontSize: 13, fontStyle: 'italic' }, navigation: { alignSelf: 'flex-start', color: '#1d4ed8', fontSize: 12, fontWeight: '800', paddingVertical: 2 }, actions: { flexDirection: 'row', flexWrap: 'wrap', gap: 12, marginTop: 5 }, reorderText: { color: '#475569', fontWeight: '800' }, disabledAction: { color: '#cbd5e1' }, edit: { color: '#2563eb', fontWeight: '700' }, delete: { color: '#dc2626', fontWeight: '700' }, voucher: { color: '#7c3aed', fontWeight: '700' }, favorite: { minWidth: 48, alignItems: 'center', justifyContent: 'center' }, favoriteText: { color: '#64748b', fontWeight: '700' }, transition: { alignSelf: 'center', maxWidth: '100%', flexDirection: 'row', alignItems: 'center', gap: 7, marginTop: -5, marginBottom: 10, paddingVertical: 7, paddingHorizontal: 12, borderRadius: 999, backgroundColor: '#eff6ff', borderWidth: 1, borderColor: '#bfdbfe' }, transitionText: { color: '#1d4ed8', fontSize: 12, fontWeight: '700', flexShrink: 1 },
});
