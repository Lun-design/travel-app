          ...
          <View style={styles.cardBody}>
            ...
            <View style={styles.content}>
              <View style={styles.cardHeader}>
                <Text style={[styles.time, { color: theme.colors.primary, backgroundColor: '#E3D8CC', borderRadius: 999, paddingHorizontal: 9, paddingVertical: 4 }]}>{scheduled?.arrivalTime ?? item.time ?? '未排定'}{scheduled?.estimated ? ' · 預估' : ''}</Text><Text numberOfLines={1} style={[styles.name, cardVisualStyles.headerName, { color: '#1A1A1A' }]}>{item.location_name}</Text>
...
const cardVisualStyles = StyleSheet.create({
  ...
  icon: { fontSize: 22 },
  headerName: { flex: 1, minWidth: 0, fontSize: 18, fontWeight: '900' },
});
