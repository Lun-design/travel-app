...
export function BudgetDashboard({ tripId, userId, expenses, members, rates, rateLabel, themeMode }: Props) {
  ...
  return <View style={[styles.container, { backgroundColor: theme.colors.surface, borderColor: theme.colors.border }]}>
    ...
    <SettlementCard tripId={tripId} userId={userId} settlements={visibleSettlements} settlementRecords={settlementRecords} members={members} labelFor={labelFor} themeMode={themeMode} onSettlementCreated={(record) => setSettlementRecords((previous) => [record, ...previous])} onSettlementDeleted={(recordId) => setSettlementRecords((previous) => previous.filter((record) => record.id !== recordId))} />
  </View>;
}
