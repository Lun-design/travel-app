...
import { ActivityIndicator, Alert, Pressable, Share, StyleSheet, Text, TextInput, View, useColorScheme } from 'react-native';
...
export function BudgetDashboard({ tripId, userId, expenses, members, rates, rateLabel, themeMode }: Props) {
  ...
  const settlementText = analytics.settlements.map((item) => `${labelFor(item.from)} → ${labelFor(item.to)} ${item.currency} ${item.amount.toFixed(2)}`).join('\n');
  ...
  async function copySettlement() {
    ...
    } catch (error) { Alert.alert('複製失敗', error instanceof Error ? error.message : '請稍後再試。'); }
  }
  ...
  return <View style={[styles.container, { backgroundColor: theme.colors.surface, borderColor: theme.colors.border }]}>
    ...
    <View style={styles.section}><View style={styles.settlementHeader}><Text style={[styles.sectionTitle, { color: theme.colors.text }]}>最少轉帳結算</Text>{analytics.settlements.length ? <Pressable accessibilityRole="button" onPress={() => void copySettlement()}><Text style={[styles.copyButton, { color: theme.colors.primary }]}>{copied ? '已複製' : '一鍵複製結算文字'}</Text></Pressable> : null}</View>{analytics.settlements.length ? analytics.settlements.map((item, index) => <Text key={`${item.from}-${item.to}-${index}`} style={[styles.settlement, { color: theme.colors.text }]}>{labelFor(item.from)} → {labelFor(item.to)} {item.currency} {item.amount.toFixed(2)}</Text>) : <Text style={[styles.muted, { color: theme.colors.muted }]}>目前沒有需要轉帳的項目。</Text>}</View>
  </View>;
}
