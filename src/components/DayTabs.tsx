...
type Props = { days: number[]; selected: number; onChange: (day: number) => void; themeMode?: ThemeMode; startDate?: string };

export function DayTabs({ days, selected, onChange, themeMode = 'system', startDate }: Props) {
  ...
  return <ScrollView style={styles.scroller} horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.wrap}>{days.map((day) => {
    ...
    return <Pressable key={day} accessibilityRole="tab" accessibilityState={{ selected: active }} accessibilityLabel={`Day ${day} ${weekday} ${dateLabel}`} onPress={() => onChange(day)} style={[styles.tab, { backgroundColor: active ? theme.colors.primary : theme.colors.surfaceMuted, borderColor: active ? theme.colors.primary : theme.colors.border }, active && styles.activeTab]}><Text style={{ color: active ? theme.colors.surface : theme.colors.text, fontWeight: active ? '800' : '700', letterSpacing: 1.1, fontSize: 11 }}>{weekday}</Text><Text style={{ color: active ? theme.colors.surface : theme.colors.text, fontWeight: active ? '900' : '700', fontSize: 15 }}>{dateLabel}</Text><Text style={{ color: active ? theme.colors.surface : theme.colors.muted, fontWeight: '600', fontSize: 9 }}>Day {day}</Text></Pressable>;
  })}</ScrollView>;
}
