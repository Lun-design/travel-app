...
import { DAY_ACTIVE_COLOR, getThemeForMode, type ThemeMode } from '@/lib/theme';
...
export function DayTabs({ days, selected, onChange, themeMode = 'system', startDate, accentColor }: Props) {
  ...
  return <ScrollView style={styles.scroller} horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.wrap}>{days.map((day) => {
    ...
    const activeColor = accentColor ?? DAY_ACTIVE_COLOR;
    const inactiveColor = '#94A3B8';
    return <Pressable key={day} accessibilityRole="tab" accessibilityState={{ selected: active }} accessibilityLabel={`Day ${day} ${weekday} ${dateLabel}`} onPress={() => onChange(day)} style={[styles.tab, { backgroundColor: active ? activeColor : 'transparent', borderColor: 'transparent', borderWidth: 0 }, active && styles.activeTab]}><Text style={{ color: active ? '#FFFFFF' : inactiveColor, fontWeight: active ? '800' : '700', letterSpacing: 1.1, fontSize: 11 }}>{weekday}</Text><Text style={{ color: active ? '#FFFFFF' : inactiveColor, fontWeight: active ? '900' : '700', fontSize: 15 }}>{dateLabel}</Text><Text style={{ color: active ? '#FFFFFF' : inactiveColor, fontWeight: '600', fontSize: 9 }}>Day {day}</Text></Pressable>;
  })}</ScrollView>;
}
const styles = StyleSheet.create({ scroller: { height: 78, flexGrow: 0, alignSelf: 'stretch', marginTop: 6, marginBottom: 8 }, wrap: { gap: 8, paddingVertical: 6, alignItems: 'center' }, tab: { height: 66, minWidth: 76, paddingHorizontal: 13, paddingVertical: 7, borderRadius: 12, borderWidth: 0, alignItems: 'center', justifyContent: 'center', gap: 2 }, activeTab: { shadowColor: '#1f1f1f', shadowOpacity: 0.1, shadowRadius: 6, shadowOffset: { width: 0, height: 2 }, elevation: 2 } });
