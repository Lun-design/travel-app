...
export function TripSettingsModal({ visible, startDate, endDate, departureTime, themeMode = 'system', onThemeModeChange, onClose, onSave }: Props) {
  ...
  useEffect(() => {
    if (visible) { setStart(startDate); setEnd(endDate); setDeparture(departureTime ?? ''); setError(''); }
  ...
  async function save() {
    ...
    try { await onSave({ start_date: trimmedStart, end_date: trimmedEnd, default_departure_time: trimmedDeparture || null }); onClose(); }
