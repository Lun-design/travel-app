...
export function buildItineraryExportText(data: ItineraryExportData): string {
  ...
  const rows = items.map((item, index) => [
    `${index + 1}. ${item.time ?? '未設定'} ${item.title}${item.category ? `（${item.category}）` : ''}`,
